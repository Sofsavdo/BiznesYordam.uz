...
class DatabaseStorage implements IStorage {
  ...
  async generateExcelExport(partnerId: string, marketplace: string, dataType: string): Promise<Buffer> {
    ...
    const worksheet = workbook.addWorksheet(`${marketplace}_${dataType}`);
      ...
      switch (dataType) {
        case 'sales_data':
          columns = [
            ...
            { header: 'Buyurtma ID', key: 'orderId', width: 15 },
          ...
          // Mock data for sales
          data = [
            { date: new Date().toLocaleDateString(), orderId: 'ORD-001', product: 'Test mahsulot', quantity: 2, price: '150,000', status: 'Topshirildi' },
            { date: new Date().toLocaleDateString(), orderId: 'ORD-002', product: 'Test mahsulot 2', quantity: 1, price: '80,000', status: 'Jarayonda' }
          ];
          break;
        ...
        case 'product_list':
          ...
          // Get actual products
          const products = await this.getProductsByPartnerId(partnerId);
          data = products.map(p => ({
            id: p.id,
            name: p.name,
            category: p.category || 'Kategoriya yo\'q',
            price: p.price,
        ...
        case 'inventory':
          columns = [
            { header: 'Mahsulot ID', key: 'productId', width: 15 },
            { header: 'Mahsulot nomi', key: 'productName', width: 30 },
            { header: 'Ombordagi miqdor', key: 'stock', width: 15 },
            { header: 'Rezerv miqdor', key: 'reserved', width: 15 },
            { header: 'Oxirgi yangilanish', key: 'lastUpdate', width: 20 }
          ];
          // Get products for inventory
          const inventoryProducts = await this.getProductsByPartnerId(partnerId);
          data = inventoryProducts.map(p => ({
            productId: p.id,
            productName: p.name,
            stock: p.quantity || 0,
            reserved: 0, // Mock reserved quantity
